...
def remove_skill(self: Event, global_unit, skill, count=-1, unconditional=False, flags=None):
    ...
    if count == 1 or str_utils.is_int(skill):
        ...
        action.do(action.RemoveSkill(unit, found_skill, count, unconditional=unconditional))
    else:
        action.do(action.RemoveSkill(unit, skill, count, unconditional=unconditional))
    if banner_flag:
...
def add_unit_map_anim(self: Event, map_anim: NID, unit: NID, speed: float=1.0, offset=None, flags=None):
    flags = flags or set()
