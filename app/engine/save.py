...
SUSPEND_LOC = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-suspend.pmeta')
...
def save_io(s_dict, meta_dict, old_slot, slot, force_loc=None, name=None):
    if name:
        save_loc = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', name, '.p')
    elif force_loc:
        save_loc = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-', force_loc, '.p')
    elif slot is not None:
        save_loc = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-', str(slot), '.p')
    ...
    # For restart
    if not force_loc:
        r_save = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-restart', str(slot), '.p')
        ...
        if meta_dict['kind'] == 'start':
            ...
            old_name = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-restart', str(old_slot), '.p')
    ...
    # For preload
    if meta_dict['kind'] == 'start':
        preload_saves = glob.glob(os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-preload-', str(meta_dict['level_nid']), '-*.p'))
        nids = [p.split('-')[-1][:-2] for p in preload_saves]
        unique_nid = str(str_utils.get_next_int('0', nids))
        preload_save = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-preload-', str(meta_dict['level_nid']), '-', unique_nid, '.p')
        preload_save_meta = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-preload-', str(meta_dict['level_nid']), '-', unique_nid, '.pmeta')
...
def load_saves():
    ...
    for num in range(0, int(DB.constants.value('num_save_slots'))):
        meta_fp = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-', str(num), '.pmeta')
...
def load_restarts():
    save_slots = []
    for num in range(0, int(DB.constants.value('num_save_slots'))):
        meta_fp = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-restart', str(num), '.pmeta')
        ss = SaveSlot(meta_fp, num)
        save_slots.append(ss)
    return save_slots
...
def get_all_saves():
    ...
    """
    save_slots = []
    name = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-turn_change-*-*.pmeta')
...
def delete_save(game_state, num: Optional[int] = None):
    ...
    if num is None:
        ...
        return
    meta_fn = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-', str(num), '.pmeta')
    save_fn = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-', str(num), '.p')
    r_save_fn = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', GAME_NID(), '-restart', str(num), '.p')
