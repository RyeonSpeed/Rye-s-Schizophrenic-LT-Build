...
class Song():
    def __init__(self, prefab):
        self.nid = prefab.nid
        self.song = pygame.mixer.Sound(os.path.dirname(__file__)+'/../../'+re.sub(r'^.*?(\w)\.ltproj', '\1\.ltproj', prefab.full_path))
        self.battle = pygame.mixer.Sound(os.path.dirname(__file__)+'/../../'+re.sub(r'^.*?(\w)\.ltproj', '\1\.ltproj', prefab.prefab.battle_full_path)) if prefab.battle_full_path else None
        self.intro = pygame.mixer.Sound(os.path.dirname(__file__)+'/../../'+re.sub(r'^.*?(\w)\.ltproj', '\1\.ltproj', prefab.prefab.intro_full_path))
...
class SoundDict(dict):
    def get(self, val):
        if val not in self:
            ...
            if sfx:
                self[val] = pygame.mixer.Sound(os.path.dirname(__file__)+'/../../'+re.sub(r'^.*?(\w)\.ltproj', '\1\.ltproj', sfx.full_path))
            else:
                return None
        return self[val]
