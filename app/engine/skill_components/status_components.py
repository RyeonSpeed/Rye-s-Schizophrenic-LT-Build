...
from app.engine.skill_info import TravelerSkill
...
class PairUpBonus(SkillComponent):
    ...
    def on_pairup(self, unit, leader):
        action.do(action.AddSkill(leader, self.value, source=unit.nid, skill_type=TravelerSkill))

    def on_separate(self, unit, leader):
        if self.value in [skill.nid for skill in leader.skills]:
            action.do(action.RemoveSkill(leader, self.value, source=unit.nid, skill_type=TravelerSkill))
